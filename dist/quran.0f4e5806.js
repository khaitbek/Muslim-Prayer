...
})({"jqA1Y":[function(require,module,exports) {
"use strict";
...
module.bundle.HMR_BUNDLE_ID = "8f53b8dc0f4e5806";
...
},{}],"493lA":[function(require,module,exports) {
...
async function getAyahsFromAPI(url, options = {}) {
    const versesData = await (0, _helperJs.makeRequest)(`${url}`, options);
...
async function getRecitationsFromApi(url, options = {}) {
    const recitationsData = await (0, _helperJs.makeRequest)(`${url}`, options);
...
async function getSingleAyah(url, id, options) {
    const ayahData = await (0, _helperJs.makeRequest)(`${url}`, options);
    const ayah = await ayahData.json();
    return ayah;
}
async function getAllJuzs() {
    const response = await (0, _helperJs.makeRequest)("https://api.quran.com/api/v4/juzs");
...
async function getSingleAyahTranslation(verseKey) {
    const response = await (0, _helperJs.makeRequest)(`https://api.quran.com/api/v4/quran/translations/55`, {
        verse_key: verseKey
    });
...
function renderSurah(ayahs, list, surah, surahNumber) {
    ...
    ayahs.forEach((ayah)=>{
        ...
        getReciterAudio(currentReciterId, ayahVerseKey).then((audio)=>{
            if (currentReciterId === 12) surahTemplateClone.querySelector("audio").src = `https:${audio.audio_files?.[0]?.url}`;
            else surahTemplateClone.querySelector("audio").src = `https://verses.quran.com/${audio.audio_files?.[0]?.url}`;
        });
...
async function getReciterAudio(reciterId, verse_key) {
    const data = await (0, _helperJs.makeRequest)(`https://api.quran.com/api/v4/quran/recitations/${Number(window.localStorage.getItem("reciterId"))}`, {
        verse_key: verse_key
    });
...
verseSelect.addEventListener("change", (evt)=>{
    if (evt.target.value === "juz") getAllJuzs();
    else getAyahsFromAPI("https://api.quran.com/api/v3/chapters");
});
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["jqA1Y","493lA"], "493lA", "parcelRequire371e")

//# sourceMappingURL=quran.0f4e5806.js.map
